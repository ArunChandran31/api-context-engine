...
DATABASE_PATH = Path(__file__).resolve().parents[1] / "api_context_engine.db"
...
def main() -> None:
    ...
    try:
        ...
        cursor.execute("PRAGMA table_info(api_specifications)")

        columns = {row[1] for row in cursor.fetchall()}

        if "user_id" not in columns:
            print("Adding user_id column to api_specifications...")
        ...
        print()
        print("The existing specifications have NOT been assigned " "to a user yet.")
        print("That will happen after Supabase authentication " "is verified.")
