...
class TestCasePromptBuilder:
    ...
    def build(
        ...
        prompt = (
            ...
            "26. Security schemes in the API Context are metadata describing "
            "authentication mechanisms. They are NOT automatically documented "
            "request headers.\n"
            "27. A security scheme such as bearerAuth does NOT by itself "
            "authorize you to generate an Authorization header.\n"
            "28. Only generate a request header when that header is explicitly "
            "documented as a request header in the API Context.\n"
            "29. For non-authentication categories such as happy, validation, "
            "edge, and errors, NEVER generate Authorization headers, bearer "
            "tokens, authentication credentials, API keys, or other "
            "authentication-related request values merely because a security "
            "scheme is documented.\n"
            "30. For non-authentication categories, the presence of bearerAuth "
            "or another security scheme MUST NOT change the generated request "
            "structure.\n"
            "31. Only an explicitly requested authentication/auth category may "
            "use authentication information from the API Context, and only "
            "when the API Context provides enough information to construct the "
            "authentication request correctly.\n"
            "32. If an authentication-specific test requires a credential but "
            "the API Context does not provide a concrete credential, use a "
            "clearly marked runtime placeholder such as '<token>' ONLY when "
            "the documented authentication scheme requires such a value for "
            "the test implementation.\n"
            "33. Never invent concrete authentication credentials or token "
            "values such as 'test-token', 'dummy-token', 'valid-token', or "
            "'testBearerToken'.\n"
            ...
            "35. Never invent concrete request-body values such as product "
            "names, prices, IDs, or boolean values unless those values are "
            "explicitly documented in the API Context.\n"
            ...
            "expired, or malformed credentials unless the corresponding "
            "behavior is explicitly documented.\n"
            "38. If the API Context documents authentication but does not "
            "document enough information to construct a grounded authentication "
            "test, do not invent missing authentication behavior. For an "
            "authentication-specific test, return a concise limitation when "
            "the missing information is necessary to perform or assert that "
            "specific authentication scenario.\n"
            "39. Security schemes are authentication metadata, not automatically "
            "documented request headers. Do not convert a security scheme such "
            "as bearerAuth into an Authorization header unless the API Context "
            "explicitly documents that header or the selected test category "
            "requires authentication. For non-authentication tests, do not "
            "invent Authorization headers, tokens, credentials, or other "
